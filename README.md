...
- 小问答、小检查、小修复：用轻量模式。
- 只想要方案：用“只规划分工，不执行”。
- 中大型迭代不走轻量模式：自动切 `Team + Implementation-Plan`，先落范围、验收和回退风险，用户批准范围后再实现。
- 跨模块、高风险、需要审查或并行工作：使用团队流程。
...
├─ 中大型 M/L 迭代
│  └─ Team + Implementation-Plan：团队讨论 -> 方案包 -> 批准 -> pre-implementation
├─ 首次接入 / 重新初始化 / 迁移 / 行业包选择
...
| `Project` | 普通实现、规划、局部修复、文档更新；判定 `M/L` 后切 Team | 项目规范、项目进度、AI 执行手册 |
| `Team` | 已初始化项目的日常团队流程、子代理、并行 agent、中大型方案迭代 | Team 运行卡，再按卡片追加调度文件 |
