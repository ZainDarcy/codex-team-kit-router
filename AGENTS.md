...
- 简单任务不派发子代理；用户明确要求团队/子代理，或任务判定为 `M/L` 并需要方案门禁时，先切 `Team + Implementation-Plan`，不得静默降级成 `Project`。
- 主线程负责需求理解、路由选择、子代理派发、结果整合、公共文件更新和最终交付。
- 子代理只处理被派发的局部任务，不修改公共文件，不启动下一层子代理。
- 中大型迭代先走 `Docs/02-执行/AI执行手册.md` 的方案门禁：先讨论和落文档，批准后再执行。
...
| `Project` | 普通规划、实现、文档更新、局部修复；判定 `M/L` 后切 `Team + Implementation-Plan` | `Docs/01-项目/项目规范.md`、`Docs/01-项目/项目进度.md`、`Docs/02-执行/AI执行手册.md` |
| `Team` | 已初始化项目的日常团队协作、子代理、并行 agent、中大型方案迭代 | `Docs/02-执行/Team运行卡.md`，再按卡片追加调度文件 |
